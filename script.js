...
let data=load(),taskFilter="todas",calendarCursor=new Date();
...
function drawLineChart(canvasId){
  ...
  ctx.strokeStyle="#203149";ctx.lineWidth=1;ctx.font="9px Inter";ctx.fillStyle="#6f7f96";
  ...
  const series=[["tarefas","#3b82f6"],["habitos","#20c97a"],["metas","#9b5cf6"],["foco","#ff9f32"]];
  series.forEach(([key,color])=>{
    ...
    ctx.stroke();
    arr.forEach((v,i)=>{const x=pad.l+(cw/6)*i,y=pad.t+ch-(v/100)*ch;ctx.fillStyle=color;ctx.beginPath();ctx.arc(x,y,3,0,Math.PI*2);ctx.fill()});
  });
}
...
function buildCalendar(containerId,date,full=false){
  ...
  const start=(first.getDay()+6)%7;const total=full?42:35;const names=["SEG","TER","QUA","QUI","SEX","SÁB","DOM"];
  let html=names.map(n=>`<div class="${full?"full-cal-head":"cal-head"}">${n}</div>`).join("");
  for(let i=0;i<total;i++){
    const d=new Date(y,m,1-start+i),iso=localISO(d),inMonth=d.getMonth()===m,tasks=data.tasks.filter(t=>t.date===iso);
    if(full){
      html+=`<div class="full-day ${inMonth?"":"other"} ${iso===localISO()?"today":""}"><b>${d.getDate()}</b><div class="day-tasks">${tasks.slice(0,3).map(t=>`<div class="day-task">${esc(t.title)}</div>`).join("")}</div></div>`;
    }else{
      html+=`<div class="cal-day ${inMonth?"":"muted-day"} ${iso===localISO()?"today":""} ${tasks.length?"has-task":""}">${d.getDate()}</div>`;
    }
  }
  c.innerHTML=html;
}
...
function renderTimeline(targetId){
  ...
  const upcoming=data.tasks.filter(t=>!t.done&&t.date>=localISO()).sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
  if(!upcoming.length){target.innerHTML=`<div class="empty">Nenhuma próxima tarefa.</div>`;return}
  const groups={};upcoming.forEach(t=>(groups[t.date]??=[]).push(t));
  target.innerHTML=Object.entries(groups).map(([date,items])=>`<div class="timeline-group"><h4>${date===localISO()?"HOJE":date===plusDays(1)?"AMANHÃ":fmtDate(date,{weekday:"long",day:"2-digit",month:"short"})}</h4>${items.map(t=>`<div class="timeline-item"><time>${t.time||"--:--"}</time><strong>${esc(t.title)}</strong><span class="tag ${t.area}">${areaNames[t.area]}</span></div>`).join("")}</div>`).join("");
}

function renderCalendar(){
  buildCalendar("miniCalendar",new Date(),false);
  buildCalendar("fullCalendar",calendarCursor,true);
  document.getElementById("calendarTitle").textContent=new Intl.DateTimeFormat("pt-BR",{month:"long",year:"numeric"}).format(calendarCursor);
  renderTimeline("upcomingTasks");renderTimeline("agendaList");
}
...
document.addEventListener("click",e=>{
  ...
  if(b.id==="todayBtn"){calendarCursor=new Date();switchPage("dashboard");renderCalendar()}
